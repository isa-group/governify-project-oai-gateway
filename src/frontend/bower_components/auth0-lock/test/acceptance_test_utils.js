...
const CURRENT_CRED_PANE_SELECTOR =
  '.auth0-lock-cred-pane:not(.horizontal-fade-exit):not(.reverse-horizontal-fade-exit)';
...
export function isShowingConfirmation(lock) {
  return !!q(lock, '.auth0-lock-confirmation:not(.slide-exit)');
}
...
export function qLocations(lock) {
  ...
  return q(
    lock,
    `.auth0-lock-select-country${AUXILIARY_PANE_SELECTOR_SUFFIX} .auth0-lock-list-code > ul > li`,
    true
  );
}
...
export function filterLocations(lock, value) {
  ...
  const input = q(
    lock,
    `.auth0-lock-select-country${AUXILIARY_PANE_SELECTOR_SUFFIX} .auth0-lock-search input`
  );
  if (!input) {
